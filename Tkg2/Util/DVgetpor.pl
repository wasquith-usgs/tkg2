...
my @options = qw ( d=s h s=s c); # these are the valid command line options
...
die "DVgetpor.pl: ".&hostname()." does not appear to be an NWIS server.\n"
   unless($_ = `nwts2rdb 2>&1` and /Usage/o);
    # The nwts2rdb command returns the 'Usage' if it is available
    
my $station = shift(@ARGV);
die "DVgetpor.pl: Please provide valid USGS station name on the command line.\n"
   unless(&_is_a_station($station));

my $computed = ($OPTS{c}) ? "-c" : ""; 
my $exe = "nwts2rdb $computed -tdv -aUSGS -n$station -d$DD -s$S -b0 -e99999999";
...
DVgetpor.pl -- A wrapper on nwts2rdb for period of record daily-value retrievals.
...
Usage: DVgetpor.pl <options> station_number
