...
my @options = qw ( h d=s s=s o=i c); # these are the valid command line options
...
die "DVlastwk.pl: ".&hostname()." does not appear to be an NWIS server.\n"
   unless($_ = `nwts2rdb 2>&1` and /Usage/o);
    # The nwts2rdb command returns the 'Usage' if it is available
    
my $station = shift(@ARGV);
die "DVlastwk.pl: Please provide valid USGS station name on the command line.\n"
...
DVlastwk.pl -- A wrapper on nwts2rdb for weekly daily-value retrievals.
  by William H. Asquith

This utility, which is shipped with the tkg2 distribution is a
handy tool for retrieving daily values from the NWIS data base in
...
Consult the nwts2rdb man page for details.  DVlastwk.pl
reports the actual nwts2rdb command forked.

Usage: DVlastwk.pl <options> station_number
       DVlastwk.pl 08167600
       DVlastwk.pl -d=19 08167600
       DVlastwk.pl -d=2 -s=00003 08167600
       DVlastwk.pl -s=00003 -d=11 -o=2 08167000
